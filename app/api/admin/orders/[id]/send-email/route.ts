...
function inferPaymentMethod(stripePaymentId: string | null) {
    if (!stripePaymentId) return 'other' as const;
    if (stripePaymentId.startsWith('paypal_')) return 'paypal' as const;
    if (stripePaymentId.startsWith('cod_')) return 'cod' as const;
        ...
        switch (type) {
            case 'confirmation':
                result = await sendOrderConfirmation({
                    ...
                    paymentMethod: inferPaymentMethod(order.stripePaymentId),
                    status: order.status
                });
                break;
